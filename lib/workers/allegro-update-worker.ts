    ...
    async (job: Job<AllegroUploadJobData>) => {
        return executeWorkerJob(job, 'allegro-upload', async (job, logger) => {
            ...
            try {
                ...
                await logger.log('Clearing existing products from database...');
                const deletedCount = await prisma.productAllegro.deleteMany({});
                await logger.log(`Deleted ${deletedCount.count} existing products`);
                ...
                for await (const row of parser) {
                    ...
                    if (sampleSize < SAMPLE_ROWS) {
                        ...
                        if (sampleSize === SAMPLE_ROWS) {
                            ...
                            await logger.log(`Estimated ~${estimatedRows} rows based on first ${SAMPLE_ROWS} rows (avg ${avgBytesPerRow.toFixed(0)} bytes/row)`);
                        }
                    }
                    ...
                    if (batch.length >= batchSize) {
                        ...
                        await logger.log(`Processed ${totalProcessed} products, ignored ${totalIgnored} rows`);
                ...
                await logger.log(`Successfully processed ${totalProcessed} products from ${totalRows} rows (${totalIgnored} ignored)`);
